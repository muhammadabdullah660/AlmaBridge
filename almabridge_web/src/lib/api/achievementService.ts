...
export const GetAllAchievements = async (): Promise<Achievement[]> => {
  try {
    const response = await axios.get<ApiAchievement[]>(
      `http://127.0.0.1:3001/api/achievements`
    );
...
export const CreateAcievement = async (
  formData: AchievementData,
  token: string
): Promise<Achievement> => {
  try {
...
function transformApiAchievementToAchievement(
