            ...
            <div class="container">
                <div class="header">
                    <div class="logo-container">
                        ...
                        <img class="logo-image" src="cid:companyLogo" alt="Company Logo">
                    </div>
...
const handleVerificationEmail = async (userId) => {
    ...
    await logAction(
        ...
        userId, 
        `A verification Code is generated for the user whose userID is ${userId}`
    );
...
const handlePasswordResetEmail = async (userId) => {
    await VerificationCode.destroy({ where: { userId } });
    const verificationCode = generateVerificationCode(8);
    ...
    const resetUrl = `http://localhost:3000/forgot-password/${verificationCode}`;
    ...
    try{
        ...
        switch(type) {
            case EMAIL_TYPES.VERIFICATION:
                emailContent = await handleVerificationEmail(userId);
                break;
            case EMAIL_TYPES.PASSWORD_RESET:
                emailContent = await handlePasswordResetEmail(userId);
                break;
            default:
        ...
        const mailOptions = {
            ...
            attachments: [
                {
                    filename: 'almabridgeLogo.png',
                    path: 'https://almabridgeworker.muhammadshahzaibijaz34.workers.dev/almabridgeLogo.png',
                    cid: 'companyLogo',
                },
            ],
            ...emailContent,
        ...
        await logAction(
            `${type === EMAIL_TYPES.VERIFICATION ? 'Verification' : 'Reset Password'} Mail Send`,
            userId,
            `A ${type} mail sent to user email whose email is ${email} and it's userID is ${userId}`
        );
    } catch (error) {
        ...
        if (error.response?.includes("550")) {
          await logAction(
            "Undeliverable Email",
            userId,
            `Failed to deliver email to ${email}: ${error.response}`,
            "failure"
          );
        } else {
          await logAction(
