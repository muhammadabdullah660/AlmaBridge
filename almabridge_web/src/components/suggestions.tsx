...
const Suggestions: React.FC = () => {
  ...
  const [scrapedSuggestions, setScrapedSuggestions] = useState<{
    profiles: Suggestion[];
  }>({
    profiles: [],
  });
  ...
  useEffect(() => {
    axios.get("http://127.0.0.1:5001/api/profiles").then((response) => {
      ...
      setScrapedSuggestions(response.data);
    });
  }, []);
  ...
  const filteredSuggestions = suggestions.filter(
    (suggestion) =>
      ...
      (departmentFilter === "all" ||
        suggestion.department === departmentFilter) &&
    ...
    <div className="flex flex-col min-h-screen">
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-col gap-2">
              ...
              <label className="text-white flex items-center">
                <input
                  type="checkbox"
                  checked={filter === "student" || filter === "all"}
                  onChange={() =>
                    setFilter(filter === "student" ? "all" : "student")
                  }
                  className="mr-2"
                />
                Student
      ...
      >
        <div className="p-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mx-auto max-w-full card-container">
          {scrapedSuggestions.profiles.map((suggestion, index) => (
            <div
              key={index}
            ...
            >
              <div className="flex flex-col items-center justify-center text-center text-white">
                ...
                <div className="mt-4">
                  ...
                  <p className="text-sm">{suggestion.headline}</p>
                  ...
                  <div className="mt-4">
                    {suggestion.education
                      ...
                      ) // Filter for UET Lahore
                      .map((edu, eduIndex) => {
                        const degreePartAfterComma =
                          edu.Degree.split(",")[1]?.trim(); // Split by comma and trim any extra spaces
                        return (
                          <div key={eduIndex} className="mb-2">
                            <h5 className="text-md font-semibold">
                              {degreePartAfterComma}
                            </h5>
