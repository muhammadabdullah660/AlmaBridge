...
const login = async (req, res) => {
  ...
  try {
    ...
    if (!user.isVerified) {
      await logAction('Login Attempt Failed', user.id, `User not verified for email: ${email}`, 'failure');
...
const verifyEmail = async (req, res) => {
  ...
  try {
    ...
    await logAction('Email Verification', user.id, `User with email: ${user.email} verified their account`);

    res.status(200).json({ message: 'Email verified successfully!' });
  } catch (error) {
    await logAction('Email Verification Failed', null, 'Failed email verification', 'failure');
